...
for size in "${SIZES[@]}"; do
    ...
    REF_TIME=$(timeout 60s ./build/parallel_matrix_vector 1 $size 2>/dev/null)
    ...
    for threads in "${THREADS[@]}"; do
        ...
        TIME=$(timeout 60s ./build/parallel_matrix_vector $threads $size 2>/dev/null)
        ...
        echo "$threads,$SPEEDUP" >> $OUTPUT_FILE
    done
done
