...
for size in "${SIZES[@]}"; do
    ...
    REF_TIME=$(timeout 60s ./build/integrate_omp 1 $size 2>/dev/null | grep "Work time:" | awk '{print $3}' || echo "TIMEOUT")
    ...
    for threads in "${THREADS[@]}"; do
        ...
        TIME=$(timeout 60s ./build/integrate_omp $threads $size 2>/dev/null | grep "Work time:" | awk '{print $3}' || echo "TIMEOUT")
